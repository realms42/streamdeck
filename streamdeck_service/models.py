...
AnyAction = Union[CommandAction, SetStateAction, ToggleAction, SetBrightnessAction]
...
class StateAppearance(BaseModel):
    ...
    font_size: Optional[int] = Field(None, ge=4, le=200)
...
class ButtonDef(BaseModel):
    ...
    def _validate_states(self) -> "ButtonDef":
        for var_name, mapping in self.states.items():
...
class DeviceConfig(BaseModel):
    serial: Optional[str] = None  # preferred selector
...
class StreamDeckConfig(BaseModel):
    ...
    def button_by_key(self, key: int) -> Optional[ButtonDef]:
        for btn in self.buttons:
            if btn.key == key:
                return btn
        return None
