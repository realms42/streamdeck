...
class StreamDeckService:
    def __init__(self, config_path: Path) -> None:
        ...
        self._deck = None
        self._config: Optional[StreamDeckConfig] = None
    ...
    def _apply_config_diff(
        self, old_cfg: StreamDeckConfig, new_cfg: StreamDeckConfig
    ) -> None:
        ...
        for key_idx, new_btn in new_map.items():
            if key_idx >= num_keys:
                log.warning("Button key %d exceeds deck key count %d — skipping", key_idx, num_keys)
                continue
    ...
    def _rerender_key(self, key_idx: int) -> None:
        ...
        key_size = self._deck.key_image_format()["size"]
        with self._render_lock:
            img = render_key(btn, self._state, key_size, self._config_dir) if btn else render_blank(key_size)
            self._set_key_image(key_idx, img)
